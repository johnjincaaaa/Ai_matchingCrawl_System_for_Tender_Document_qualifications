...
def get_projectId(session,down_articleId):
...
def download(session: requests.Session,projectID):
    ...
    response = session.get(verify_url, params=params)
    ...
    headers = {
        ...
        "Pragma": "no-cache",
        "Referer": "https://www.zcygov.cn/bid-enroll/_procurement_/acquirepurfile/launch/7328227397386108950?type=4",
        "Sec-Fetch-Dest": "empty",
        "Sec-Fetch-Mode": "cors",
        "Sec-Fetch-Site": "same-origin",
...
if __name__ == '__main__':
    ...
    down_articleId = get_download_articleId(articleId='35oztRwHQ01JYY8dulkEkQ==')
    print(down_articleId)
    login_session = login()
    if login_session:
        print(login_session)
        projectId = get_projectId(session=login_session,down_articleId=down_articleId)
        print('=============', projectId)
        if projectId is not None:
            download(login_session,projectId)
