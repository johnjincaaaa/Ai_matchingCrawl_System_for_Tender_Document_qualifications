...
AI_CONFIG = {
    ...
    "bid_security_prompt_path": os.path.join(BASE_DIR, "prompts", "bid_security_prompt.txt"),
    ...
    # 投标保证金语义判断配置（用AI语义理解代替关键词匹配，避免"不需要投标保证金"被误判）
    "bid_security_check": {
        "enable": True,  # True：用AI语义判断是否需要投标保证金；False：回退到关键词预过滤
    },
...
B_RULE_CONFIG = {
    "default_rules": [
        ...
        {
            "rule_name": "业绩要求",
            "trigger_condition": "项目要求有类似项目业绩，或要求提供业绩证明材料（如合同复印件、中标/成交通知书、验收报告等），或要求提供相关中标（成交）公告的下载网页/网址/截图/链接以佐证业绩",
            "conclusion": "无论是否拥有类似项目业绩，均满足本条款要求，无需提供任何业绩相关证明材料。【业绩公告网址/截图豁免】即使评分项要求提供中标（成交）公告的下载网页、网址、链接或截图，这类是招标/交易平台公开发布、我方真实存在的业绩记录，网址/截图我方可正常提供，判定为满足、【不触发】政府官方网站备案要求排除规则（业绩公告发布平台不必然是政府官方网站，且属可正常提供的公开信息，非我方不具备的官方核验资质）",
            "rule_type": "业绩类",
            "is_active": 1
        },
        {
        ...
        },
        {
            ...
            "conclusion": "该要求需政府官方网站备案/可查询验证。【真实持证豁免】若所需证书公司A类库中确实真实持有、且属'证书本身即为评分项所要求那一类'的严格匹配(如要求质量/环境/职业健康安全管理体系认证，A类库正好持有对应证书)，则核验可正常通过，判定为满足、不触发本排除规则(此类通用体系认证ISO9001/14001/45001本就登记于cnca.cn，真实持证即为有效)。【社保及自有人员材料豁免】若要求的是本单位自有人员的社保/养老保险缴纳清单或证明(含加盖社保缴费证明专用章)、劳动合同、在职证明等材料，属投标人为自有人员如实缴纳后可正常取得的真实材料(分公司为员工缴纳的社保同样认可)，可正常提供，判定为满足、不触发本排除规则(详见'社保及自有人员材料通用判定'规则)。【业绩公告网址/截图豁免】若要求的是提供中标(成交)公告的下载网页/网址/链接/截图以佐证业绩，这类是招标/交易平台公开发布、我方真实存在的业绩记录(发布平台不必然是政府官方网站)，网址/截图可正常提供，判定为满足、不触发本排除规则(详见'业绩要求'规则)。【仍触发排除的情形】所需材料公司并不真实持有，或仅靠'语义相似/通用分类/可视为子类'把通用体系认证硬套为其不能覆盖的具体产品认证/能效认证/专业业务范围认证，或要求对非自有/拟外聘/第三方人员提供社保、或要求在【政府官方网站】(如信用中国、政府采购网、能效标识网等)备案/公开查询验证公司本不具备的资质——此类需提供真实的政府官方网站备案材料，判0分",
            "rule_type": "排除类",
            "is_active": 1
        },
        {
        ...
        {
            "rule_name": "社保及自有人员材料通用判定",
            "trigger_condition": "评分项要求提供本单位【自有人员】的社保 / 养老保险缴纳清单或缴纳证明、社保缴费记录、社保缴费证明（含加盖社保缴费证明专用章、社保机构出具、人社 / 社保部门出具等表述），或岗位人员的劳动合同、在职证明、工资发放记录等人员在职关系证明材料。注意：这类材料是投标人为自己实际用工人员如实缴纳后可正常取得的真实材料（分公司为员工缴纳的社保同样认可），与'公司并不真实持有、需官方公开可查'的第三方核验材料有本质区别。",
            "conclusion": "我公司可为自有项目人员如实提供社保 / 养老保险缴纳清单或证明（可加盖社保缴费证明专用章）、劳动合同、在职证明等材料，均判定为满足，【不触发】政府官方网站备案要求排除规则（此类为投标人自有人员的真实可得材料，非公司不具备的第三方公开核验资质）。仅当评分项额外要求对'非自有 / 拟外聘 / 第三方人员'提供社保，或要求通过政府官方网站公开查询验证公司本不具备的资质时，才另行按排除规则判定。",
            "rule_type": "人员资质类",
            "is_active": 1
        },
        {
            "rule_name": "承诺响应",
            "trigger_condition": "项目要求有承诺响应、提供承诺、提供说明和承诺、承诺文件、承诺书、书面承诺；或评分项属于投标人可自主承诺即可满足的事项——按语义理解识别，典型如：质保期/质量保证期限延长、售后服务年限、服务响应时间、到场/到货时间、交货期/供货期承诺、免费维护期、培训场次等，此类均视为承诺响应（不要求原文出现“承诺”二字）",
        ...
        {
            "rule_name": "技术参数响应",
            "trigger_condition": "对所投产品的技术参数、功能、性能、配置、规格等响应情况进行打分的评分项，包含以下任一情形：1）参数/功能偏离表、参数响应表打分；2）'所投产品具备某功能/特性/参数即得分'（如掉电记忆、智能防直吹、节能等级、材质、尺寸、能耗、接口等）；3）此类评分项要求提供的证明材料为投标人可自行准备的产品自有资料——产品样本、技术规格书、官方说明书、产品彩页、参数配置表、厂家技术文件、功能截图/照片等。注意：本规则【不适用】于要求政府官方网站备案/可查询验证、或第三方检测报告并明确要求可查验的情形（那些分别走'政府官方网站备案要求排除规则'/'检测报告都具备'规则）。",
            "conclusion": "我公司可通过选配满足要求的产品并提供对应产品样本、技术规格书、官方说明书、彩页、参数配置表等产品自有资料予以响应，该类产品参数/功能响应评分项均判定为满足（按满分计）。",
            "rule_type": "技术参数类",
            "is_active": 1
        },
        {
        ...
        },
        {
            ...
            "trigger_condition": "项目客观评分条目包含2个及以上独立计分小项，且各子项需要【具体证明材料】方能得分(如体系认证多项计分、软件著作权多项计分、人员多证书分项计分)。注意：本规则仅约束“需材料证明”的多子项条目，不适用于已被其他B类规则判定为“无需材料即满足”的条目",
            "conclusion": "本条目禁止整体直接判定满分，各子项独立核算得分，同一条目总分为所有子项得分累加值，单项子项无材料不得分，仅豁免“无材料直接整条目计0分”否决规则。【重要例外】若该条目已触发“业绩要求”“承诺响应”“技术参数响应”等判定为“无需提供材料即满足”的B类规则，则以该规则结论为准判为满足，不适用本拆分否决（业绩多条累加、承诺类多子项均按满足计满分）",
            "rule_type": "计分逻辑约束规则",
            "is_active": 1
        },
        {
...
# 进AI前的前置过滤配置（确定性过滤，不调用AI，节省时间与token）
PRE_FILTER_CONFIG = {
    # 非招标文件过滤：命中即跳过AI分析，标记为已排除
    "non_tender_filter": {
        "enable": True,
        # 结果/流程类标记：只匹配标题（project_name），因招标正文常出现"合同/验收"等词，扫全文会误杀真招标
        "title_keywords": [
            "中标结果", "中标公告", "成交公告", "成交结果", "结果公告",
            "更正公告", "更正通知", "澄清公告", "答疑", "补充公告",
            "终止公告", "终止通知", "废标", "流标", "中止",
            "合同公告", "合同履行", "验收公告", "验收结果", "履约验收",
        ],
        # 附件类标记：匹配文件名（file_path）+ 标题，这类通常是独立文件
        "filename_keywords": [
            "图纸", "工程量清单", "报名资料", "报名表", "报名登记",
        ],
    },
    # 投标保证金过滤：需要保证金的项目直接中断分析、设为不推荐（不调AI）
    "bid_security_filter": {
        "enable": True,
        # 明确的“投标保证金”类关键词 → 直接判定需要投标保证金
        "strong_keywords": ["投标保证金", "投标担保金", "投标保函"],
        # 泛化关键词“保证金”：命中后需排除下方 exempt_prefixes 修饰的非投标类保证金
        "generic_keyword": "保证金",
        # 豁免前缀：紧邻“保证金”之前若出现这些词，视为非投标保证金（履约/质量等），不拦截
        # 注：质保金 本身不含“保证金”，天然不受影响
        "exempt_prefixes": [
            "履约", "质量", "质保", "农民工", "工资", "廉政", "廉洁",
            "安全", "预付", "材料", "缺陷", "维修", "临时", "解冻", "退还",
        ],
        # 上下文回看窗口：判断 exempt_prefixes 时向前取的字符数
        "prefix_window": 6,
        # 否定表述（防误杀）：命中任一则视为"无需投标保证金"，不拦截
        # 全文子串匹配，覆盖常见"不要求/不收取/免收/无需/为0/为无"等表述
        "negation_keywords": [
            # 不要求 / 不需要 / 不收取 类
            "不要求投标保证金", "不需要投标保证金", "不需缴纳投标保证金",
            "不收取投标保证金", "不收投标保证金", "不再收取投标保证金", "不予收取投标保证金",
            "不设投标保证金", "不设置投标保证金", "未设投标保证金", "未设置投标保证金",
            "不提交投标保证金", "不缴纳投标保证金",
            # 免收 / 免缴 类
            "免收投标保证金", "免缴投标保证金", "免交投标保证金", "免除投标保证金",
            # 无需 / 无须 / 毋须 类
            "无需缴纳投标保证金", "无需提交投标保证金", "无须缴纳投标保证金",
            "无须提交投标保证金", "毋须缴纳投标保证金", "无投标保证金",
            # 金额为 0 / 为无 类
            "投标保证金：0", "投标保证金:0", "投标保证金为0", "投标保证金 0",
            "投标保证金：无", "投标保证金:无", "投标保证金为无", "投标保证金（无）",
            "投标保证金金额：0", "投标保证金金额:0", "投标保证金金额为0",
        ],
        # 命中后写入的最终判定与排除原因前缀
        "decision": "不推荐",
    },
