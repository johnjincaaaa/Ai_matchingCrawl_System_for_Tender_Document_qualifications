...
class OllamaService(AIService):
    ...
    def initialize(self, config):
        try:
            ...
            log.info(f"初始化Ollama服务，模型: {model_name}, 地址: {base_url}, CUDA: {'启用' if cuda_enabled else '禁用'}")
            ...
            log.error(f"Ollama服务初始化失败: {str(e)}")
            return False
    ...
    def health_check(self):
        try:
            import requests
            base_url = self.llm.base_url if hasattr(self.llm, 'base_url') else "http://localhost:11434"
